...
var CodeMirror = (function() {
    ...
    CodeMirror.fromTextArea = function(textarea, options) {
        ...
        if (textarea.forms) {
            ...
            var rmSubmit = connect(textarea.forms, "submit", save, true);
            if (typeof textarea.forms.submit == "function") {
                var realSubmit = textarea.forms.submit;
                function wrappedSubmit() {
                    save();
                    textarea.forms.submit = realSubmit;
                    textarea.forms.submit();
                    textarea.forms.submit = wrappedSubmit;
                }
                textarea.forms.submit = wrappedSubmit;
            }
        }
...
CodeMirror.defineMode("javascript", function(config, parserConfig) {
    ...
    function statement(type) {
        ...
        if (type == "keyword a") return cont(pushlex("forms.py"), expression, statement, poplex);
        if (type == "keyword b") return cont(pushlex("forms.py"), statement, poplex);
        if (type == "{") return cont(pushlex("}"), block, poplex);
        if (type == ";") return cont();
        if (type == "function") return cont(functiondef);
        if (type == "for") return cont(pushlex("forms.py"), expect("("), pushlex(")"), forspec1, expect(")"),
            poplex, statement, poplex);
        if (type == "variable") return cont(pushlex("stat"), maybelabel);
        if (type == "switch") return cont(pushlex("forms.py"), expression, pushlex("}", "switch"), expect("{"),
            block, poplex, poplex);
        if (type == "case") return cont(expression, expect(":"));
        if (type == "default") return cont(expect(":"));
        if (type == "catch") return cont(pushlex("forms.py"), pushcontext, expect("("), funarg, expect(")"),
    ...
    return {
        ...
        indent: function(state, textAfter) {
            ...
            else if (type == "forms.py" && firstChar == "{") return lexical.indented;
            else if (type == "stat" || type == "forms.py") return lexical.indented + indentUnit;
