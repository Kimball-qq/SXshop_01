...
from SXshop.settings import private_key_path, ali_public_key_path,appid
...
#     :return:
#     """
alipay=AliPay(appid=appid, ##appid，用沙箱的话会给你个
              app_notify_url='http://39.103.172.179:8000/alipay/return/',   ##默认回调url，需要外网才能跳转，
              app_private_key_string=private_key_path,  ##本地私钥

              ##支付宝的公钥，验证支付回传消息时使用,不是自己生成的公钥，
              alipay_public_key_string = ali_public_key_path,
              sign_type="RSA2",  ##RSA 或 RSA2  (当你在支付宝验证过密钥正确性还报不匹配时可以试试换RSA2或RSA)
              debug=False , ##默认False，若开启则使用沙盒环境用True
              )
    # return alipay
"""
AliPay()参数：
